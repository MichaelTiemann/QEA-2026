...
% 1. Default safe limits (respecting MATLAB's 2^31 indexing limit)
if nargin < 5 || isempty(max_elements)
    max_elements = 2^31;
end

% 2. Safe defaults for vfoptions struct if missing
...
n_e = [];
if isfield(vfoptions, 'n_e'); n_e = vfoptions.n_e; end
N_e  = max(1, prod(n_e(n_e > 0)));

n_semiz = [];
if isfield(vfoptions, 'n_semiz'); n_semiz = vfoptions.n_semiz; end
N_semiz = max(1, prod(n_semiz(n_semiz > 0)));

N_ze = N_z * N_e * N_semiz;

% 4. Core Tensor-Bridge Footprint (Choices x States^2 for endogenous)
...
if N_ze > 1
    if N_semiz > 1 && (total_elements / N_semiz) <= max_elements
        lowmemory = 1; elements_per_chunk = total_elements / N_semiz; return;
    elseif N_z > 1 && (total_elements / N_z) <= max_elements
        lowmemory = 1; elements_per_chunk = total_elements / N_z; return;
    elseif N_e > 1 && (total_elements / N_e) <= max_elements
        lowmemory = 1; elements_per_chunk = total_elements / N_e; return;
    elseif (total_elements / N_ze) <= max_elements
...
% Hard limit fallback
warning('Grid exceeds safe index/VRAM limits even with Level 5 maximum slicing.');
lowmemory = 5;
