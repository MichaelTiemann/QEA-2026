% No medical, only unemployment

function [z_grid,pi_z]=LifeCycleModel21_ExogShockFn1(agej,Jr)
% z takes two values:
...
elseif agej==Jr % Want a special one-off transition to determine an initial distribution of the 'new' medical expense shocks
    ...
    z_grid=[0;0];
    pi_z=[0.2,0.8;0.3,0.7]; % Medical expense shocks are reasonably rare and not very persistent
end
